...
## 96. GUI: устойчивый AI fallback, компактные действия, чистый лог и русские подписи

Промт:
- во время боя исправить ошибку `Policy selected an illegal action: target_index 0 is not valid for Greataxe`
- сделать меню действий ещё компактнее, чтобы кнопки не растягивались на пол-экрана
- меню ресурсов переместить над меню действий и убрать из него имя активного существа
- убрать из боевого GUI-лога строки `Reward breakdown: ...`
- перевести названия карт и врагов на русский

Изменения:
- `BattleAIService`:
  - если загруженная policy выбирает нелегальное действие, GUI делает fallback на mask-aware агента для текущего шага
  - ошибка несовместимого действия больше не прерывает демонстрационный бой
- `ActionPanel`:
  - кнопки действий получили фиксированную компактную ширину
  - кнопки прижаты влево и больше не растягиваются на всю доступную ширину
- `BattleScreen`:
  - панель ресурсов перенесена над панелью действий
  - из строки ресурсов убрано имя активного существа
- `CombatLogWidget`:
  - `Reward breakdown:` фильтруется только на уровне GUI-отображения
  - внутренний `CombatEnvironment.action_log` не меняется, чтобы не ломать обучение и тесты reward
- `BattleSetupService` и `ui.text`:
  - добавлены русские display-labels для карт
  - добавлены русские display-labels для базовых врагов
  - технические ключи карт продолжают использоваться для загрузки JSON

Проверка:
- `python -m pytest src/tests/test_inference_services.py src/tests/test_battle_setup_service.py src/tests/test_gui_smoke.py src/tests/test_rewards.py`
- `python -m pytest`
- результат полного прогона: `355 passed`

## 97. BG3-like компактная панель способностей и перевод имён монстров в логе

Промт:
- посмотреть `orig.png` со скрином Baldur's Gate 3
- сделать панель способностей похожей по принципу:
  - компактные квадратные кнопки
  - все способности должны помещаться без прокручивания
  - полное название способности показывать при наведении
  - категории способностей расположить колонками
- исправить лог боя, где названия монстров всё ещё отображались латиницей

Изменения:
- `ActionPanel`:
  - полностью заменён старый layout со scroll area
  - категории действий теперь отображаются отдельными колонками
  - действия отображаются квадратными слотами `32x32`
  - полное название действия вынесено в tooltip
  - для частых действий добавлены короткие символы-иконки
  - целевые действия помечаются дополнительным маркером
- `theme.py`:
  - добавлен тёмный стиль квадратных ability-кнопок
  - добавлен стиль колонок способностей
- `ui.text`:
  - добавлен перевод `Fire Elemental Simple`
  - `translate_battle_log` делает повторный проход через `ru_sentence` после шаблонных замен, чтобы имена монстров переводились и в сложных строках лога
- `BattleScreen`:
  - активный участник в строке состояния проходит через `ru_sentence`

Проверка:
- `python -m pytest src/tests/test_gui_smoke.py src/tests/test_battle_setup_service.py src/tests/test_inference_services.py`
- `python -m pytest`
- результат полного прогона: `356 passed`

## 98. Стабильная панель действий без прокрутки и явное завершение ручного хода

Промт:
- цветовое оформление панели способностей должно соответствовать остальному интерфейсу
- интерфейс не должен сжиматься и разжиматься при выборе действий
- если действие недоступно, показывать его бледным disabled-слотом, а не убирать с панели
- при ручном управлении игроками завершать ход только явно через кнопку завершения хода
- расположить кнопки более горизонтально в колонках
- исправить tooltip, которые оставались на латинице

Изменения:
- `ActionPanel`:
  - базовые слоты действий всегда присутствуют на панели
  - недоступные действия отображаются disabled и больше не меняют размер layout
  - категории остались колонками, но действия внутри колонок раскладываются более горизонтально
  - прокрутка отсутствует
  - tooltip строится через `ru_sentence`
- `theme.py`:
  - оформление ability-слотов возвращено к общей светлой теме интерфейса
  - disabled-слоты получили бледное состояние
- `CombatEnvironment`:
  - добавлен флаг `auto_end_turn_enabled=True`
  - стандартное поведение AI/training не изменено
- `BattleScreen`:
  - при ручном действии временно отключается auto-end-turn
  - ход ручного игрока завершается только через `EndTurnAction`
- `ui.text`:
  - добавлены русские подписи для базовых stable-слотов
  - добавлены русские подписи оружия и некоторых боевых терминов

Проверка:
- `python -m pytest src/tests/test_gui_smoke.py`
- `python -m pytest`
- результат полного прогона: `357 passed`

## 99. Фикс смерти активного участника от opportunity attack и новая группировка способностей

Промт:
- бой завис, когда орк на своём ходу отошёл от воина, получил провоцированную атаку и умер
- после смерти активного орка кнопка следующего хода не работала
- исправить зависание на ходу умершего активного существа
- поменять расположение способностей:
  - всё, что связано с перемещением, в колонку `Перемещение`
  - атакующие способности, толчок и захват в колонку `Атакующие способности`
  - заклинания и особые классовые способности в колонку `Заклинания`
  - предметы инвентаря в колонку `Инвентарь`
  - для реализованных предметов рисовать отдельные слоты, а количество показывать в tooltip

Изменения:
- `CombatEnvironment.step()`:
  - после успешного действия вызывает `_skip_unavailable_active_actor()`
  - если активный актёр умер или стал недееспособен внутри своего действия, ход сразу передаётся следующему доступному участнику
  - исправлен сценарий смерти от opportunity attack во время собственного движения
- `ManualActionBuilder`:
  - заменены группы ручных действий на:
    - `Movement`
    - `Attack Abilities`
    - `Spells`
    - `Inventory`
    - `End Turn`
  - атаки, Help, Grapple, Shove, Stabilize и Search попадают в атакующие способности
  - Dash/Disengage/Dodge/Hide попадают в перемещение
  - заклинания и class features попадают в заклинания
  - все предметы попадают в инвентарь
- `ActionPanel`:
  - стабильные слоты обновлены под новые смысловые колонки
  - inventory-слоты включают реализованные предметы
  - tooltip предмета показывает количество
- `ui.text`:
  - добавлены русские подписи новых групп

Проверка:
- `python -m pytest src/tests/test_environment.py src/tests/test_manual_action_builder.py src/tests/test_gui_smoke.py`
- `python -m pytest`
- результат полного прогона: `358 passed`
## 95. Уплотнение боевого GUI и снятие лимита характеристик 15

Промт:
- убрать ограничение на повышение характеристик до 15
- в симуляторе боя сделать кнопки действий значительно меньше
- вместо текстовой карточки ресурсов показывать компактные индикаторы в стиле BG3:
  - action
  - bonus action
  - reaction
  - movement
  - spell slots
  - class resources
- убрать из симуляции боя окно с подробными характеристиками персонажа

Изменения:
- `StatEditor`:
  - базовые характеристики теперь можно поднимать до 20
  - point-buy продолжает считать очки от базы 8 и показывает перерасход, если игрок выходит за 27 очков
- `ActionPanel`:
  - минимальная ширина снижена до 220
  - кнопки действий сделаны ниже и компактнее
  - в одной строке группы показывается до 5 действий
- `BattleScreen`:
  - убран `CreatureStatusPanel` из правой панели
  - справа остался только порядок инициативы
  - ресурсы активного участника показываются в нижней панели компактными маркерами
- `theme.py`:
  - добавлен стиль для компактной панели ресурсов
  - добавлен компактный стиль для кнопок боевых действий
